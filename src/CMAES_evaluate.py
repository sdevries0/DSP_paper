...
from NDE import NDE, ParameterReshaper
...
class Evaluator:
    """Evaluator for NDE policies in control tasks.
        ...
        """
    def __init__(self, env, parameter_reshaper: ParameterReshaper, state_size: int, dt0: float, solver=diffrax.Euler(), max_steps: int = 16**4, stepsize_controller: diffrax.AbstractStepSizeController = diffrax.ConstantStepSize()) -> None:
        self.env = env
    ...
    def evaluate_control_loop(self, model: NDE, x0: Array, ts: Array, target: float, process_noise_key: PRNGKey, obs_noise_key: PRNGKey, params: Tuple) -> Tuple[Array, Array, Array, Array, float]:
        ...
        sol = diffrax.diffeqsolve(
            ...
            event=diffrax.Event(self.env.cond_fn_nan), stepsize_controller=self.stepsize_controller, args = (env, model, obs_noise_key, target)
        )
    ...
    def _drift(self, t, x_a, args):
        ...
        da = model.update(jnp.concatenate([y, u, a, target])) #Compute hidden state updates
