...
class Evaluator:
    ...
    def evaluate_LQG(self, x0: Array, ts: Array, target: Array, process_noise_key: PRNGKey, obs_noise_key: PRNGKey, params: Tuple) -> Tuple[Array, Array, Array, Array]:
        ...
        sol = diffrax.diffeqsolve(
            system, solver, ts[0], ts[-1], dt0, init, saveat=saveat, args=(x_star, u_star, L), adjoint=diffrax.DirectAdjoint(), max_steps=self.max_steps, stepsize_controller=self.stepsize_controller
        )
