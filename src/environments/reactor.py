...
class StirredTankReactor(EnvironmentBase):
    ...
    def __init__(self, process_noise: float = 0.0, obs_noise: float = 0.0, n_obs: int = 3, n_targets: int = 1, external_f: callable = lambda t: 0.0) -> None:
        ...
        self.init_lower_bounds = jnp.array([275, 300, 0.5])
        self.init_upper_bounds = jnp.array([350, 375, 1.0])
    ...
    def initialize_parameters(self, params: Tuple[Array, Array, Array, Array, Array, Array, Array, Array], ts: Array) -> None:
        ...
        self.G = jnp.eye(self.n_var) * jnp.array([4, 8, 0.05]) 
        self.V = self.G * self.process_noise

        self.C = jnp.eye(self.n_var)[:self.n_obs]
        self.W = self.obs_noise * jnp.eye(self.n_obs) * (jnp.array([4, 20, 0.1])[:self.n_obs])
    ...
    def sample_param_change(self, key: jrandom.PRNGKey, batch_size: int, ts: Array, low: float, high: float) -> Array:
        ...
        decay_factors = 1.01*jnp.ones(batch_size,)
    ...
    def sample_params(self, batch_size: int, mode: str, ts: Array, key: jrandom.PRNGKey) -> Tuple[Array, Array, Array, Array, Array, Array, Array, Array]:
        ...
        if mode == "Constant":
            ...
            Cp = jrandom.uniform(keys[1], (batch_size,), minval=250, maxval=350)
            dHr = jrandom.uniform(keys[2], (batch_size,), minval=-50000, maxval=-45000)
            UA = jrandom.uniform(keys[3], (batch_size,), minval=35000, maxval=65000)
    ...
    def sample_init_states(self, batch_size: int, key: jrandom.PRNGKey) -> Tuple[Array, Array]:
        ...
        targets = jrandom.uniform(target_key, shape=(batch_size, self.n_targets), minval=300, maxval=450)
        return x0, targets
    ...
    def drift(self, t: float, state: Array, args: Tuple) -> Array:
        ...
        """
        Tc, T, c = state
        control = jnp.clip(args, 0, 300)
        _u = control
        control = jnp.squeeze(control)
