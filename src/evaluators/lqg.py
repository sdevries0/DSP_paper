...
class LQG:
    def __init__(self, env, dt0, solver, max_steps):
        self.env = env
    ...
    def compute_ricatti(self, A, b, Q, R):
        ...
        sol = diffrax.diffeqsolve(
            system, diffrax.Euler(), 0, 10, 0.01, jnp.zeros((2,2)), max_steps=self.max_steps
        )
        return sol.ys[0]
    ...
    def evaluate_LQG(self, x0: Sequence[float], ts: Sequence[float], target: float, process_noise_key, obs_noise_key, params: Tuple):
        ...
        sol = diffrax.diffeqsolve(
            system, solver, ts[0], ts[-1], self.dt0, init, saveat=saveat, args=(L, env, obs_noise_key, targets), adjoint=diffrax.DirectAdjoint(), max_steps=self.max_steps
        )
        ...
        costs = env.fitness_function(x, u, targets.evaluate(ts), ts)

        return x, y, u, costs
    
    def drift(self, t, variables, args):
        L, env, obs_noise_key, target = args
