...
class StaticEvaluator:
    ...
    def __call__(self, candidate: Array, data: Tuple, tree_evaluator: Callable) -> float:
        ...
        """
        _, _, _, fitness = jax.vmap(self.evaluate_control_loop, in_axes=[None, 0, None, 0, 0, 0, 0, None])(candidate, *data, tree_evaluator)
    ...
    def evaluate_control_loop(self, candidate: Array, x0: Array, ts: Array, target: float, process_noise_key: jrandom.PRNGKey, obs_noise_key: jrandom.PRNGKey, params: Tuple, tree_evaluator: Callable) -> Tuple[Array, Array, Array, Array, float]:
        ...
        sol = diffrax.diffeqsolve(
            ...
            args=(env, candidate, obs_noise_key, targets, tree_evaluator), stepsize_controller=self.stepsize_controller, throw=False
        )

        xs = sol.ys[:,:self.latent_size]
        _, ys = jax.lax.scan(env.f_obs, obs_noise_key, (ts, xs))
        us = jax.vmap(lambda y, tar: tree_evaluator(candidate, jnp.concatenate([y, tar])), in_axes=[0,0])(ys, targets.evaluate(ts)[:,None])
    ...
    def _drift(self, t, x_a, args):
        env, readout, obs_noise_key, target, tree_evaluator = args
        x = x_a[:self.latent_size]

        _, y = env.f_obs(obs_noise_key, (t, x)) #Get observations from system
        u = tree_evaluator(readout, jnp.concatenate([y, jnp.atleast_1d(target.evaluate(t))]))
    ...
    def _diffusion(self, t, x_a, args):
        ...
        return env.diffusion(t, x, jnp.array([0])) #Only the system is stochastic