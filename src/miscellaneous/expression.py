...
class Expression:
    ...
    def __init__(self, obs_size = 0, state_size = 0, control_size = 0, target_size = 0, unary_functions = [], condition = None, operators_prob = None, leaf_prob = None):
        ...
        log = OperatorNode(lambda x: jnp.log(x), "log", 1)
        squareroot = OperatorNode(lambda x: jnp.sqrt(x), "√", 1)
        unary_dict = {"sin": sine, "cos": cosine, "tanh": tanh, "exp": exp, "log": log, "√": squareroot}
        for unary_f in unary_functions:
            self.operators.append(unary_dict[unary_f])
